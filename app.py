"""
UK Property Search App - Main Streamlit Application
"""
...
def render_sidebar_search():
    ...
    with st.sidebar:
        ...
        with st.expander("Advanced: Postcode + Street"):
            st.caption("Provide BOTH postcode district AND street name.")
            col_district, col_street = st.columns(2)
            with col_district:
                postcode_district = st.text_input(
                    "Postcode District",
                    placeholder="e.g. SW1A, NG8",
                    help="The first part of a postcode"
                )
            with col_street:
        ...
        if error_message:
            ...
            with st.spinner("Searching properties..."): 
                ...
                st.session_state.last_search = {
                    "area": area, 
                    "query": query or f"{postcode_district} {street_name}".strip()
                }
